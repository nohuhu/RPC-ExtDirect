...
For methods that take 0 arguments, it will be the first argument after the
class name.
...
Environment object will be passed in the C<_env> key.
...
user authorization, logging, cleanup, testing, etc. Think of the Hooks
as a poor man's method wrappers without L<Moose's|Moose> power
...
C<file_uploads> key with value set to arrayref of file hashrefs. Each file
...
Use CPAN RT tracker or Github tracker (preferable) to open issues.

=head1 SEE ALSO

You can find additional information in the following Web site links:
...
on versions 2.0-3.0 of the RPC::ExtDirect suite of modules.
