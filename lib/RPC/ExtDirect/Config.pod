...
Create a new Config instance and populate it with key/value pairs from
%options. If an option is not specified, a default value will be assumed;
see L</OPTIONS> for more information.

The supported legacy package global variables will be read before the
%options is processed; thus any option passed directly to constructor
...
Create and return a new instance of Config as a copy of the current one.
This method only does shallow copy, i.e. any config option that is a
reference will refer to the same object.

=item set_options(%options)

Apply the passed options to the current Config instance.

=item get_router_path()

Return the current L</router_path> value.

This method is deprecated and is provided only for backwards compatibility
with RPC::ExtDirect 1.x and 2.x.

=item get_poll_path()

Return the current L</poll_path> value.

This method is deprecated and is provided only for backwards compatibility
with RPC::ExtDirect 1.x and 2.x

=item get_remoting_var()

Return the current L</remoting_var> value.

This method is deprecated and is provided only for backwards compatibility
with RPC::ExtDirect 1.x and 2.x.

=item get_polling_var()

Return the current L</polling_var> value.
...
L<RPC::ExtDirect::Serializer> module. This option only affects
...
Turn debugging on/off for L<RPC::ExtDirect::Request> module. When
...
Turn debugging on/off for L<RPC::ExtDirect::Router> module. When
...
Class name to be used instead of L<RPC::ExtDirect::Exception> when
...
in serialization method of L<RPC::ExtDirect::Serializer> module.
...
Class name to be used when instantiating Exception objects thrown
in deserialization method of L<RPC::ExtDirect::Serializer> module.
...
Class name to be used when instantiating Exception objects thrown
in L<RPC::ExtDirect::Request>, when a Request is being processed.
...
Class name to be used instead of L<RPC::ExtDirect::Request> when
...
deserialization method of L<RPC::ExtDirect::Serializer> module.
...
Class name to be used instead of L<RPC::ExtDirect::Request::PollHandler>
when instantiating Request objects in L<RPC::ExtDirect::EventProvider>
module.
...
Class name to be used instead of L<RPC::ExtDirect::Serializer> when
...
Class name to be used when instantiating Serializer objects used to
serialize Request results in L<RPC::ExtDirect::EventProvider> module.
...
Class name to be used when instantiating Serializer objects used to
serialize Request results in L<RPC::ExtDirect::Router> module.
...
Class name to be used when instantiating Serializer objects used to
deserialize incoming Request data in L<RPC::ExtDirect::Router> module.
...
L<RPC::ExtDirect::Router>. This config option is not used directly by
...
L<RPC::ExtDirect::EventProvider>. Similar to L</router_class>,
