...
C<RPC::ExtDirect::API>, created at compile time and populated by the
...
populated with L<Actions and Methods|/"ACTIONS AND METHODS"> at the package
compilation time, and should be available globally thoughout the application.
...
L<RPC::ExtDirect::API> provides several public methods:
...
by the L</HOOK_TYPES> method. See L<RPC::ExtDirect/HOOKS> for more
...
This parameter is mandatory, and it can take one of the following forms:
